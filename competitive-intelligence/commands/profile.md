...
Read `{workspace_root}/config.yaml` to get `profile_analyzers`. For each name in the list, check `agents/analyzers/0_ANALYZERS.md` for `status: active`. Run all active analyzers in parallel.
