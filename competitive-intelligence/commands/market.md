...
- Most recent `{workspace_root}/syntheses/market-*.md` (if any exists) — passed as `previous_synthesis` to surface what changed

### Step 3 — Run market-synthesizer

Follow the full analysis process in `agents/synthesizers/market-synthesizer.md`, passing:
- `all_profiles`: full content of all profiles in `{workspace_root}/profiles/`
- `our_product_context`: content of `{workspace_root}/{product_name}.md` (or empty string)
