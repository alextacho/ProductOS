...
After fetching the changelog page, read the date and title of the most recent entry at the top of the page. Compare it against the most recent row in the `**Recent releases:**` table in `current_section`.

- If the top entry's date **and** title match the most recent row in `current_section`: return `current_section` unchanged, appending one line: `_No new releases since last run ([date])._` Then stop — skip Steps 2–4.
- If they differ, or if you can't determine a match with confidence: continue to Step 2.

This check is only valid when there is a previous section to compare against. On first run, `current_section` is empty — skip this check entirely.
