...
Read `workspace/competitors.yaml`. Load the full list of competitors: name, website, tier, last_updated.

Read `context/signal-weights.md`. You will pass this path to the delta detector.
...
1. Read `workspace/config.yaml` — get the `profile_analyzers` list
...
- `our_product_context`: read from `workspace/ourproduct.md` if it exists; otherwise pass empty string
...
The distributor reads `context/distribution.yaml`. It always returns a note — either confirming delivery, listing what wasn't connected, or prompting the user to set up targets. Append that note to the brief.
...
```
workspace/competitors.yaml                       — entity index (read + update last_updated)
context/signal-weights.md                        — signal weight definitions (pass to delta-detector)
workspace/ourproduct.md                          — our product description for synthesis (user-provided)
workspace/profiles/[name].md                     — current-state profiles (Direction, SWOT, Our Read, Run History)
workspace/snapshots/[name]/[YYYY-MM-DD].md       — full run snapshots (extractors + delta + synthesis)
workspace/syntheses/[YYYY-MM-DD].md              — market-level synthesis outputs
workspace/briefs/[YYYY-MM-DD].md                 — output briefs (dispatched by brief-distributor)
context/distribution.yaml                        — distribution target config (committed default; edit to configure targets)
workspace/schedule.yaml                          — cron schedule config (gitignored; managed by /ci:schedule)
agents/extractors/0_EXTRACTORS.md                  — extractor index (check status: active before spawning)
agents/synthesizers/0_SYNTHESIZERS.md              — synthesizer index (reference only — profile synthesizer always runs; market synthesizers are on-demand via commands)
agents/extractors/[name].md                      — extractor agents
skills/delta-detector.md                         — delta detection skill
skills/profile-builder.md                        — profile builder skill (system — writes Current State + Direction)
agents/analyzers/0_ANALYZERS.md                  — analyzer index (cross-reference with workspace/config.yaml)
agents/analyzers/[name]-analyzer.md              — analyzer skills (each writes one ## Analysis — [Framework] section)
workspace/config.yaml                            — pipeline config (profile_analyzers list)
